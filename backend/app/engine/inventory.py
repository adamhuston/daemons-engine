...
from .world import World, WorldPlayer, WorldItem, ItemTemplate, PlayerId, ItemId
...
def can_add_item(world: World, player_id: PlayerId, template_id: str, quantity: int = 1) -> Tuple[bool, str]:
    """
    ...
    # Check weight
    new_weight = calculate_inventory_weight(world, player_id) + (template.weight * quantity)
    ...
    if template.max_stack_size == 1:
        if inventory.current_slots >= inventory.max_slots:
            return False, f"Inventory full! ({inventory.current_slots}/{inventory.max_slots} slots)"
...
def remove_item_from_inventory(world: World, player_id: PlayerId, item_id: ItemId) -> WorldItem:
    """
...
def equip_item(world: World, player_id: PlayerId, item_id: ItemId) -> Optional[ItemId]:
    """
...
def unequip_item(world: World, player_id: PlayerId, item_id: ItemId) -> None:
    ...
    if not item.is_equipped():
        raise InventoryError(f"Item is not equipped")
    
    slot = item.equipped_slot
    if player.equipped_items.get(slot) != item_id:
        raise InventoryError(f"Item is not equipped in expected slot")
