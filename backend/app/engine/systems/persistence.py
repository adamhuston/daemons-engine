...
from typing import TYPE_CHECKING, Dict, Set, Any, Callable, Awaitable
...
class DirtyState:
    ...
    def has_dirty_entities(self) -> bool:
        ...
        return bool(
            self.players or self.rooms or self.npcs or 
            self.items or self.triggers
        )
...
class StateTracker:
    ...
    async def _save_room(self, session: "AsyncSession", room_id: str) -> None:
        ...
        try:
            ...
            """), {
                ...
                "dynamic_exits": str(room.dynamic_exits) if hasattr(room, 'dynamic_exits') else "{}",
    ...
    def schedule_item_decay(self) -> None:
        ...
        if not self.ctx.time_manager:
            print("[StateTracker] TimeEventManager not available, cannot schedule item decay")
            return
        ...
        print(f"[StateTracker] Scheduled item decay check every {self.DECAY_CHECK_INTERVAL}s")
    ...
    def schedule_periodic_save(self) -> None:
        ...
        """
        if not self.ctx.time_manager:
            print("[StateTracker] TimeEventManager not available, cannot schedule periodic saves")
            return
    ...
    def stop_periodic_save(self) -> None:
        ...
        if hasattr(self, '_decay_event_id') and self._decay_event_id and self.ctx.time_manager:
