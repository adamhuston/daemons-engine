...
class ClanCommand:
    ...
    def handle_create(self, player_id: str, player_name: str, args: str) -> List[Event]:
        ...
        if not args.strip():
            return [self.ctx.msg_to_player(
                player_id,
                "**Usage:** clan create <name> [description]"
            )]
        ...
        if player_id in clan_system.player_to_clan:
            return [self.ctx.msg_to_player(
                player_id,
                "You are already in a clan. Leave it first."
            )]
        
        # Check if clan name already exists
        if any(clan.name.lower() == clan_name.lower() for clan in clan_system.clans.values()):
            return [self.ctx.msg_to_player(
                player_id,
                f"A clan with the name '{clan_name}' already exists."
            )]
        
        try:
            import asyncio
            clan_info = asyncio.run(clan_system.create_clan(clan_name, player_id, description))
    ...
    def handle_invite(self, player_id: str, player_name: str, args: str) -> List[Event]:
        ...
        if target_id in clan_system.player_to_clan:
            return [self.ctx.msg_to_player(
                player_id,
                f"{target_name} is already in a clan."
            )]
        ...
        try:
            ...
            events = [
                self.ctx.msg_to_player(
                    player_id,
                    f"You invited {target_name} to clan '{clan.name}'."
                ),
                self.ctx.msg_to_player(
                    target_id,
    ...
    def handle_join(self, player_id: str, player_name: str, args: str) -> List[Event]:
        ...
        if clan_id not in clan_system.clans:
            return [self.ctx.msg_to_player(
                player_id,
                f"Clan with ID {clan_id} not found."
            )]
        ...
        if player_id not in clan.pending_invites:
            return [self.ctx.msg_to_player(
                player_id,
                f"You were not invited to clan '{clan.name}'."
            )]
        ...
        try:
            ...
            events = [
                self.ctx.msg_to_player(
                    player_id,
                    f"You joined clan '{clan.name}'."
                ),
                self.ctx.event_dispatcher.clan_message(
                    clan_id,
                    player_id,
                    player_name,
                    f"{player_name} joined the clan."
                ),
            ]
            return events
        except ValueError as e:
    ...
    def handle_leave(self, player_id: str, player_name: str, args: str) -> List[Event]:
        ...
        try:
            ...
            events = [
                self.ctx.msg_to_player(
                    player_id,
                    f"You left clan '{clan.name}'."
                ),
                self.ctx.event_dispatcher.clan_message(
                    clan_id,
                    player_id,
                    player_name,
                    f"{player_name} left the clan."
                ),
            ]
            return events
        except ValueError as e:
    ...
    def handle_promote(self, player_id: str, player_name: str, args: str) -> List[Event]:
        """Promote a clan member (leader only)."""
        if not args.strip():
            return [self.ctx.msg_to_player(
                player_id,
                "**Usage:** clan promote <player_name>"
            )]
        ...
        if clan.leader_id != player_id:
            return [self.ctx.msg_to_player(
                player_id,
                "Only the clan leader can promote members."
            )]
        ...
        if target_id not in clan.members:
            return [self.ctx.msg_to_player(
                player_id,
                f"{target_name} is not in your clan."
            )]
        ...
        try:
            ...
            events = [
                self.ctx.msg_to_player(
                    player_id,
                    f"You promoted {target_name} to officer."
                ),
                self.ctx.msg_to_player(
                    target_id,
                    f"You were promoted to officer in clan '{clan.name}'."
                ),
                self.ctx.event_dispatcher.clan_message(
                    clan_id,
                    player_id,
    ...
    def handle_members(self, player_id: str, player_name: str, args: str) -> List[Event]:
        """List clan members."""
        clan_system = self.ctx.clan_system
        ...
        for member_id, rank in clan.members.items():
            if member_id != clan.leader_id:
                ...
                if member:
                    rank_name = "Officer" if rank == 3 else "Member" if rank == 2 else "Initiate"
    ...
    def handle_disband(self, player_id: str, player_name: str, args: str) -> List[Event]:
        ...
        try:
            ...
            for member_id in list(clan_system.player_to_clan.keys()):
                if clan_system.player_to_clan[member_id] == clan_id:
                    ...
                    if member_id in self.ctx.world.players:
                        self.ctx.msg_to_player(
                            member_id,
                            f"Clan '{clan.name}' has been disbanded."
                        )
            ...
            return [self.ctx.msg_to_player(
                player_id,
                f"You disbanded clan '{clan.name}'."
            )]
        except ValueError as e:
            return [self.ctx.msg_to_player(player_id, f"Error: {str(e)}")]
...
def register_clan_commands(router) -> None:
    ...
    def cmd_clan(engine, player_id: str, args: str) -> List[Event]:
        ...
        if subcommand == "create":
            ...
            return [engine.ctx.msg_to_player(
                player_id,
