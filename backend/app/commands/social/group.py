...
class GroupCommand:
    ...
    def handle_invite(self, player_id: str, player_name: str, args: str) -> List[Event]:
        ...
        if not args.strip():
            return [self.ctx.msg_to_player(
                player_id,
                "**Usage:** group invite <player_name>"
            )]
        ...
        if group.leader_id != player_id:
            return [self.ctx.msg_to_player(
                player_id,
                "Only the group leader can invite players."
            )]
        ...
        try:
            ...
            events = [
                self.ctx.msg_to_player(
                    player_id,
                    f"You invited {target_name} to the group."
                ),
                self.ctx.msg_to_player(
                    target_id,
    ...
    def handle_members(self, player_id: str, player_name: str, args: str) -> List[Event]:
        ...
        for mid in group.members:
            ...
            if member:
                member_names.append(f"  {member.name}" + (" (leader)" if mid == group.leader_id else ""))
    ...
    def handle_rename(self, player_id: str, player_name: str, args: str) -> List[Event]:
        ...
        try:
            ...
            return [self.ctx.msg_to_player(
                player_id,
                f"Renamed group from '{old_name}' to '{new_name}'."
            )]
        except ValueError as e:
            return [self.ctx.msg_to_player(player_id, f"Error: {str(e)}")]
    ...
    def handle_disband(self, player_id: str, player_name: str, args: str) -> List[Event]:
        ...
        if group.leader_id != player_id:
            return [self.ctx.msg_to_player(
                player_id,
                "Only the group leader can disband the group."
            )]
        ...
        for mid in members:
            events.append(self.ctx.msg_to_player(
                mid,
                f"Group '{group_name}' was disbanded by {player_name}."
            ))
...
def register_group_commands(router) -> None:
    ...
    def cmd_group(engine, player_id: str, args: str) -> List[Event]:
        ...
        if subcommand == "create":
            ...
            return [engine.ctx.msg_to_player(
                player_id,
                f"Unknown group subcommand: {subcommand}"
            )]
