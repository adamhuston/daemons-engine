...
class TellCommand:
    ...
    def handle_tell(self, player_id: str, player_name: str, args: str) -> List[Event]:
        ...
        if player_id in ignored_list:
            return [self.ctx.msg_to_player(
                player_id,
                f"{target.name} is ignoring your tells."
            )]
    ...
    def handle_reply(self, player_id: str, player_name: str, args: str) -> List[Event]:
        ...
        if not player.player_flags or "last_tell_from" not in player.player_flags:
            return [self.ctx.msg_to_player(
                player_id,
                "No one has told you anything recently."
            )]
        ...
        if target_id not in self.ctx.world.players:
            return [self.ctx.msg_to_player(
                player_id,
                f"Player '{target_name}' is no longer online."
            )]
    ...
    def handle_ignore(self, player_id: str, player_name: str, args: str) -> List[Event]:
        ...
        if target_id in ignored_list:
            return [self.ctx.msg_to_player(
                player_id,
                f"You are already ignoring {target_name}."
            )]
        ...
        return [self.ctx.msg_to_player(
            player_id,
            f"You are now ignoring {target_name}."
        )]
    
    def handle_unignore(self, player_id: str, player_name: str, args: str) -> List[Event]:
        ...
        if target_id not in ignored_list:
            return [self.ctx.msg_to_player(
                player_id,
                f"You are not ignoring {target_name}."
            )]
        ...
        return [self.ctx.msg_to_player(
            player_id,
            f"You are no longer ignoring {target_name}."
        )]
