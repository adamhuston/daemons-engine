...
class FactionCommand:
    ...
    def handle_join(self, player_id: str, player_name: str, args: str) -> List[Event]:
        ...
        if not faction.player_joinable:
            return [self.ctx.msg_to_player(
                player_id,
                f"You cannot join '{faction.name}' (NPC-only faction)."
            )]
        ...
        if standing.joined_at is not None:
            return [self.ctx.msg_to_player(
                player_id,
                f"You are already a member of '{faction.name}'."
            )]
    ...
    def handle_leave(self, player_id: str, player_name: str, args: str) -> List[Event]:
        ...
        if standing.joined_at is None:
            return [self.ctx.msg_to_player(
                player_id,
                f"You are not a member of '{faction.name}'."
            )]
        ...
        try:
            ...
            return [self.ctx.msg_to_player(
                player_id,
                f"You have left '{faction.name}'."
            )]
        except ValueError as e:
            return [self.ctx.msg_to_player(player_id, f"Error: {str(e)}")]
    
    def handle_standing(self, player_id: str, player_name: str, args: str) -> List[Event]:
        ...
        if not standings:
            return [self.ctx.msg_to_player(
                player_id,
                "You have not joined any factions yet."
            )]
