...
from app.engine.systems.clan_system import ClanSystem, ClanInfo, ClanMemberInfo
from app.engine.systems.context import GameContext
from app.engine.world import World, WorldRoom, WorldPlayer
from app.models import Clan, ClanMember, Player
...
class TestClanCreation:
    ...
    async def test_create_clan_success(self, clan_system):
        ...
        with patch.object(clan_system, '_create_clan_in_db', new_callable=AsyncMock):
    ...
    async def test_create_clan_player_not_in_clan(self, clan_system):
        ...
        with patch.object(clan_system, '_create_clan_in_db', new_callable=AsyncMock):
            ...
            clan_system.player_to_clan["player_0"] = clan.clan_id
    ...
    async def test_create_clan_persists_to_db(self, clan_system):
        ...
        with patch.object(clan_system, '_create_clan_in_db', mock_create):
            clan = await clan_system.create_clan("Dragons", "player_0")
...
class TestClanMemberManagement:
    ...
    async def test_invite_player_to_clan(self, clan_system):
        ...
        # Invite player_1
        with patch.object(clan_system, '_invite_player_in_db', new_callable=AsyncMock):
            await clan_system.invite_player(clan.clan_id, "player_1")
        
        assert "player_1" in clan.members
        assert clan.members["player_1"] == 1  # RANK_INITIATE
    ...
    async def test_invite_already_in_clan(self, clan_system):
        ...
        with patch.object(clan_system, '_create_clan_in_db', new_callable=AsyncMock):
            ...
            clan_system.clans[clan.clan_id] = clan
            clan_system.player_to_clan["player_0"] = clan.clan_id
            clan_system.player_to_clan["player_1"] = 999  # In different clan
    ...
    async def test_remove_player_from_clan(self, clan_system):
        ...
        with patch.object(clan_system, '_create_clan_in_db', new_callable=AsyncMock):
            clan = await clan_system.create_clan("Dragons", "player_0")
            clan_system.clans[clan.clan_id] = clan
            clan.members["player_1"] = 1  # RANK_INITIATE
            clan_system.player_to_clan["player_0"] = clan.clan_id
            clan_system.player_to_clan["player_1"] = clan.clan_id
        
        # Remove player_1
        with patch.object(clan_system, '_remove_player_in_db', new_callable=AsyncMock):
            await clan_system.remove_player(clan.clan_id, "player_1")
    ...
    async def test_cannot_remove_leader(self, clan_system):
        ...
        with pytest.raises(ValueError, match="Cannot remove leader"):
            with patch.object(clan_system, '_remove_player_in_db', new_callable=AsyncMock):
                await clan_system.remove_player(clan.clan_id, "player_0")
...
class TestClanRanksAndPermissions:
    ...
    def test_can_invite_leader(self, clan_system):
        ...
        clan = ClanInfo(
            ...
            experience=0,
            created_at=None,
            members={"player_0": 4},  # RANK_LEADER
            pending_invites=set(),
        )
    ...
    def test_can_invite_officer(self, clan_system):
        ...
        clan = ClanInfo(
            ...
            experience=0,
            created_at=None,
            members={"player_0": 4, "player_1": 3},  # player_1 is RANK_OFFICER
            pending_invites=set(),
        )
        clan_system.clans[1] = clan
    ...
    def test_member_cannot_invite(self, clan_system):
        ...
        clan = ClanInfo(
            ...
            experience=0,
            created_at=None,
            members={"player_0": 4, "player_1": 2},  # player_1 is RANK_MEMBER
            pending_invites=set(),
        )
        clan_system.clans[1] = clan
    ...
    async def test_promote_player(self, clan_system):
        ...
        with patch.object(clan_system, '_promote_player_in_db', new_callable=AsyncMock):
            await clan_system.promote_player(1, "player_1")
        
        assert clan.members["player_1"] == 3  # RANK_OFFICER
...
class TestClanLevelingAndContribution:
    ...
    async def test_add_contribution(self, clan_system):
        ...
        initial_exp = clan.experience
        with patch.object(clan_system, '_add_contribution_in_db', new_callable=AsyncMock):
            await clan_system.add_contribution(1, "player_1", 25)
        
        assert clan.experience == initial_exp + 25
    ...
    async def test_clan_level_progression(self, clan_system):
        ...
        with patch.object(clan_system, '_update_clan_in_db', new_callable=AsyncMock):
            level = clan_system._calculate_clan_level(clan)
        
        # Even with 500 exp, should still be relatively low level
        # (actual levels depend on progression formula)
        assert level >= 1
...
class TestClanDatabaseLoading:
    ...
    async def test_load_clans_from_db_empty(self, clan_system):
        ...
        with patch.object(clan_system.db_session_factory, '__call__', return_value=mock_session):
    ...
    async def test_load_clans_from_db_with_data(self, clan_system):
        ...
        mock_member1.rank = 4  # RANK_LEADER
        
        mock_member2 = MagicMock()
        mock_member2.player_id = "player_1"
        mock_member2.rank = 2  # RANK_MEMBER
        
        mock_clan.members = [mock_member1, mock_member2]
        
        mock_session = AsyncMock()
        result = MagicMock()
        result.scalars = MagicMock(return_value=[mock_clan])
...
class TestClanDisband:
    ...
    async def test_disband_clan(self, clan_system):
        ...
        clan_system.clans[1] = clan
        clan_system.player_to_clan["player_0"] = 1
        clan_system.player_to_clan["player_1"] = 1
        
        with patch.object(clan_system, '_disband_clan_in_db', new_callable=AsyncMock):
    ...
    async def test_disband_nonexistent_clan(self, clan_system):
        ...
        with pytest.raises(ValueError, match="not found"):
            with patch.object(clan_system, '_disband_clan_in_db', new_callable=AsyncMock):
                await clan_system.disband_clan(999)
...
class TestClanEdgeCases:
    ...
    async def test_player_to_clan_mapping_consistency(self, clan_system):
        """Test that player_to_clan mapping stays consistent."""
        with patch.object(clan_system, '_create_clan_in_db', new_callable=AsyncMock):
            clan1 = await clan_system.create_clan("Dragons", "player_0")
            clan_system.clans[clan1.clan_id] = clan1
            clan_system.player_to_clan["player_0"] = clan1.clan_id
        
        with patch.object(clan_system, '_invite_player_in_db', new_callable=AsyncMock):
            await clan_system.invite_player(clan1.clan_id, "player_1")
            clan_system.player_to_clan["player_1"] = clan1.clan_id
        
        # player_1 should be in clan1
        assert clan_system.player_to_clan["player_1"] == clan1.clan_id
...
class TestClanSystemPerformance:
    ...
    def test_can_invite_o1_lookup(self, clan_system):
        ...
        clan = ClanInfo(
            ...
            experience=0,
            created_at=None,
            members={f"player_{i}": (4 if i == 0 else 2) for i in range(100)},
            pending_invites=set(),
        )
        clan_system.clans[1] = clan
