...
down_revision: Union[str, Sequence[str], None] = '186cf284ed62'
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('attack_damage_min', sa.Integer(), nullable=False, server_default='1'),
        sa.Column('attack_damage_max', sa.Integer(), nullable=False, server_default='5'),
        sa.Column('attack_speed', sa.Float(), nullable=False, server_default='3.0'),
        sa.Column('experience_reward', sa.Integer(), nullable=False, server_default='10'),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['template_id'], ['npc_templates.id'], name=op.f('fk_npc_instances_template_id_npc_templates')),
        sa.ForeignKeyConstraint(['room_id'], ['rooms.id'], name=op.f('fk_npc_instances_room_id_rooms')),
        sa.ForeignKeyConstraint(['spawn_room_id'], ['rooms.id'], name=op.f('fk_npc_instances_spawn_room_id_rooms')),
        ...
        try:
            ...
            for spawn in spawn_data['spawns']:
                ...
                result = connection.execute(
                    sa.select(npc_templates.c.max_health).where(npc_templates.c.id == template_id)
                ).fetchone()
                
                if not result:
                    print(f"  Warning: NPC template {template_id} not found, skipping spawn")
                    continue
