...
def upgrade() -> None:
    ...
    op.create_table(
        'security_events',
        sa.Column('id', sa.String(), nullable=False),
        sa.Column('account_id', sa.String(), nullable=True),  # Nullable for failed login attempts
...
def downgrade() -> None:
    ...
    with op.batch_alter_table('players', schema=None) as batch_op:
        ...
        batch_op.drop_constraint('fk_players_account_id_user_accounts', type_='foreignkey')
