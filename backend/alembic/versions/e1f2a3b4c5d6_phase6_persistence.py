...
down_revision: Union[str, None] = 'd9e5f8a7b2c3'
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('template_id', sa.String(), sa.ForeignKey('npc_templates.id'), nullable=False),
        sa.Column('current_room_id', sa.String(), sa.ForeignKey('rooms.id'), nullable=True),
        sa.Column('current_hp', sa.Integer(), nullable=True),
        sa.Column('is_alive', sa.Boolean(), nullable=False, server_default='1'),
        sa.Column('owner_player_id', sa.String(), sa.ForeignKey('players.id'), nullable=True),  # For companions
    ...
    op.add_column('item_instances', sa.Column('decay_minutes', sa.Integer(), nullable=True, server_default='60'))
    
    # === Extend npc_templates table ===
    # Add persist_state flag for NPCs that should survive restarts
    op.add_column('npc_templates', sa.Column('persist_state', sa.Boolean(), nullable=True, server_default='0'))
