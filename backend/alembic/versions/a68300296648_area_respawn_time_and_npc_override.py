...
down_revision: Union[str, Sequence[str], None] = '7d4e8f2a3b1c'
...
def upgrade() -> None:
    ...
    op.add_column('areas', sa.Column('default_respawn_time', sa.Integer(), nullable=False, server_default='300'))
...
def downgrade() -> None:
    ...
    with op.batch_alter_table('npc_instances') as batch_op:
        batch_op.add_column(sa.Column('respawn_time_new', sa.Integer(), nullable=False, server_default='300'))
    
    # Copy values, defaulting NULL to 300
    op.execute("UPDATE npc_instances SET respawn_time_new = COALESCE(respawn_time, 300)")
