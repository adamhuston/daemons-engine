...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['leader_id'], ['players.id'], name='fk_clans_leader_id'),
    ...
    op.create_table(
        ...
        sa.Column('rank', sa.String(), nullable=False),  # leader|officer|member|initiate
        sa.Column('joined_at', sa.Float(), nullable=False),
        sa.Column('contribution_points', sa.Integer(), nullable=False, server_default='0'),
        sa.PrimaryKeyConstraint('id'),
        sa.ForeignKeyConstraint(['clan_id'], ['clans.id'], ondelete='CASCADE', name='fk_clan_members_clan_id'),
        sa.ForeignKeyConstraint(['player_id'], ['players.id'], name='fk_clan_members_player_id'),
